...
- **graceful, ACTIONABLE N/A** — AD is multi-master, so `failover` (no DC failover; FSMO move/seize), `scale-out` (DC add/remove = terraform), `backup`/`restore` (system-state = `wbadmin`/`ntdsutil`, out-of-band), `cert-rotate` (LDAPS = the security overlay; an unguarded NTDS restart is refused), and `chaos` (DC kill risks the auth plane) each return a `Result.Fail` that names the right out-of-band tool — never a silent stub.
